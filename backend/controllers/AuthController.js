    ...
    try {
      ...
      if (findUser) {
        ...
        const permission = await prisma.permissions.findMany({
          ...
          where: { role_id: parseInt(findUser.roles.id) },
        });
        const routesActions = {};
        const allRoutes = [];

        permission?.forEach(({ actions, routes }) => {
          const action = actions?.name;
          const path = routes?.path;

          if (path) {
            routesActions[path] = routesActions[path] || [];
            routesActions[path].push(action);
